...
// import { multerMiddleware } from '@/infra/config/http/middlewares/multer';
// import {
//   Application,
...
//       const { controller, method, path, httpMethod, middlewares } = route;

//       const mountedRoute = middlewares?.length
//         ? [
//             ...middlewares.sort((a, b) => {
//               if (a === this.middlewares.auth) {
//                 return -1;
//               }
//               if (b === this.middlewares.auth) {
//                 return 1;
//               }
//               return 0;
//             }),
...
//           ]
//         : [methodAdapterToExpress(controller, method)];
//       if (httpMethod === 'get') router.get(path, ...mountedRoute);
//       if (httpMethod === 'post') router.post(path, ...mountedRoute);
//       if (httpMethod === 'put') router.put(path, ...mountedRoute);
//       if (httpMethod === 'delete') router.delete(path, ...mountedRoute);
//     }
//     app.use(router);
